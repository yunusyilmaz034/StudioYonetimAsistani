...
| `turnstile.reopened` | `{ deviceId, direction, secondsSinceCrossing }` | **OR-79 (2026-09-14).** The arm did not turn; the member scanned the same side again within 45 s of a turnstile crossing that had not been reopened yet, and the arm was turned once more. **Not a new check-in** — occupancy, attendance and credits are untouched. The last `checkIns` record gets `reopenedAt` in the same batch; a second reopen of the same crossing is refused. Subject is the member. |
| `device.restart_requested` | `{ deviceId, reason }` | **Firmware v1.4 (2026-09-14).** Reception or the owner asked the turnstile box to restart itself instead of cutting its power. `reason` is required — a restart is the trace of a fault. Refused for a deactivated device. The command itself travels on the existing `turnstileCommands/{deviceId}` channel (`action: 'restart'`), written after the event. Restarts only the ESP box, never the turnstile's own board. Subject is the device (branch aggregate). |
| `member.entry_refused` | `{ branchId, deviceId, reason }` | **2026-09-08 · OR-53, widened by OR-78 (2026-09-14).** The turnstile refused an ENTRY. `reason` is a closed enum: `no_active_membership` (no package valid today) · `no_credits_left` (every valid package used up, nothing reserved) · `no_entries_left` (limited fitness allowance used up). A member with a class reservation around now is let in. No state changes; the code is not spent. Widening the enum is additive — every earlier event is `no_active_membership` and means what it meant. |
...
| `class_session.guest_arrived` | `{ holdId }` | **OR-76 (2026-09-14).** The non-member a seat was held for (Multisport) was let through by reception from the hold's row. An observation, kept apart from `class_session.seat_held` (the intention). `related: { classSessionId, seatHoldId }`. **No name, no card number.** Only on the session's own studio-local day (`guest_arrival_not_today`); a second entry writes nothing. Not a `member.checked_in` — the guest never enters occupancy. The arm itself is `turnstile.opened_manually` carrying the same `related`. |
| `staff.week_plan_draft_saved` | `{ weekStart, staffCount, blockCount }` | **OR-77 (2026-09-14).** Reception (or the owner) saved a week's shift-plan draft. `subject: { kind: 'staffWeekPlan', id: weekStart }` — the plan is approved as a WEEK, not per person. Saving an identical draft writes nothing. |
| `staff.week_plan_submitted` | `{ weekStart, staffCount, blockCount, changedDays }` | **OR-77.** Sent for approval. `changedDays` = (staff, day) cells that differ from the published plan. A draft identical to the published plan is refused (`week_plan_unchanged`). |
| `staff.week_plan_approved` | `{ weekStart, version, blocks: [{ staffUserId, date, start, end }] }` | **OR-77.** Owner-only. The draft becomes the published plan staff see. Carries **every block of the week** so "what was the plan that week" survives later edits. No names — opaque ids, dates, `HH:MM`. One block per day; no overnight shifts. |
| `staff.week_plan_returned` | `{ weekStart, reason }` | **OR-77.** Owner-only; `reason` mandatory. The plan becomes a draft again; the **published plan is untouched**, so staff keep seeing the approved hours. |
| `staff.leave_document_added` | `{ leaveId, staffUserId, documentId, pageCount }` | **OR-77 karar 4 (2026-09-14).** A sick note (photo or PDF) attached to a `rapor` leave. **Health data:** no Storage path and no content in the payload — a path in the log could never be erased. Only the leave's owner or the studio owner may add; only they may read (5-minute signed URL). Refused on a cancelled or rejected leave; 1–10 pages. The record lives at `staffLeaves/{id}/documents/{docId}`, a subcollection no client rule matches. |
| `staff.leave_document_removed` | `{ leaveId, staffUserId, documentId, reason }` | **OR-77.** A correction, never a silent delete (#9): `reason` mandatory. The record is deleted in the same transaction; the Storage objects are deleted after it commits (best effort). |
| `staff.shift_plan_membership_set` | `{ staffUserId, included }` | **OR-77 (2026-09-14).** Owner-only. Whether a staff member appears in the weekly shift plan — who is planned is a decision, not a role (a shared reception login, the owner's own trainer account). Stored as `inShiftPlan` on `/staff`; absent means included. Same value writes nothing. |
