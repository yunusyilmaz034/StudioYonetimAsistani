...
| **AD-66** | **The kasa requirement does not apply to the `migration` actor.** A cash or POS payment requires an open drawer (v1.24) — *unless* `actor.type === 'migration'`, in which case it is recorded with its **true** method and a **truthful** `drawerId: null`. **Rejected alternatives, and the lie each one tells:** a synthetic "migration drawer" fabricates a gün sonu that never happened, inside the one control the owner relies on to catch theft; re-labelling the payment `bank_transfer` falsifies how the member actually paid; skipping the payment leaves every migrated member appearing to owe what she already paid. **Rationale:** the drawer is a control over *the act of taking cash at the desk*. A migration takes no cash — it records cash taken years ago, in a system that had no drawer at all. This is not a finance exception; it is a rule about *who the actor is*, and `migration` is a first-class principal precisely so the domain can say this out loud (#5). It is unreachable by a human, a client or an AI: the actor is derived server-side and exists only inside `tools/migration`, run by hand. *(Owner, 2026-07-13 · v1.26)* |

| **AD-67** | **KVKK/GDPR erasure is `member.erased`, and the reason is a CLOSED ENUM.** `{ memberId, reason, erasedAt }` — an opaque id, one of `kvkk_request \| duplicate \| test_data \| legal_requirement \| owner_request`, and a timestamp. **Rejected: a free-text reason** — it is the last place PII can hide in a permanent log (*"Ayşe Yılmaz'ın avukatı aradı"*), and the log is forever. The human's explanation lives on the member's **tombstone in state**, where it can itself be erased. The member record is **tombstoned, never deleted**: deleting it would break every join (her reservations, payments and credits all point at it) and turn a lawful erasure into a corrupt database. Actor: **`platform_admin` only** — erasure is a break-glass act, not an operation; reception must not be able to make a member disappear, and neither must the owner mid-argument. **Idempotent**: a second erasure emits no event. **`/events` is not touched, and does not need to be** — PII has never entered a payload (#6), so her behaviour survives, the ledger still balances, and none of it says who she was. *A system that had put her name in its events would face a choice here between obeying the law and keeping its own books.* *(Owner, 2026-07-13 · v1.26)* |
