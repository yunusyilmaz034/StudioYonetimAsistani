...
class FakeRepo implements ReservationRepository {
  ...
  async listCancelledResolvedBetween(
    ...
    return [...this.reservations.values()].filter(
      (r) => r.status === 'cancelled' && (r.resolvedAt ?? 0) >= from && (r.resolvedAt ?? 0) < to,
    )
  }
  ...
  async applyNote(_c: TenantContext, reservation: Reservation, events: readonly NewEvent[]): Promise<void> {
